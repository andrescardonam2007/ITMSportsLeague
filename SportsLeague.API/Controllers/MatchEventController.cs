...
public class MatchEventController : ControllerBase
{
    ...
    int matchId, MatchResultRequestDTO dto)
    {
        try
        {
            ...
            var created = await _matchEventService.RegisterResultAsync(matchId,
           result);
    ...
    [HttpGet("goals")]
    public async Task<ActionResult<IEnumerable<GoalResponseDTO>>> GetGoals(int
   matchId)
    {
        try
        {
    ...
    [HttpGet("cards")]
    public async Task<ActionResult<IEnumerable<CardResponseDTO>>> GetCards(int
   matchId)
    {
        try
        {
